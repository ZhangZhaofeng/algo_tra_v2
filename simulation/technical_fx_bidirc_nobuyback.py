...
class GMMA:
    ...
    def simulate(self, num=100, periods="1m" ,end_offset=0):
        ...
        retreat=2 #0: using sell_price;
                  #1: using buy_price;
                  #2: using average of buy and sell
        leverage = 1.0
        ...
        for t in range(61, len(all)):
            ...
            if retreat==0:
                ...
                reverse_price = (sell_price+buy_price)/2
            else:
                print("Wrong retreat")
                break
            ...
            if not short and not long:
                if all[t][3] < sell_price and mode==0:   #low price is lower than sell_price
                    ...
                    if all[t][2] > reverse_price:
                        reverse_price = reverse_price + slide
                        # short over
                        short = False
                ...
                if all[t][2] > reverse_price:  # close price is higher than reverse_price
                    reverse_price=reverse_price+slide
                    # short over
                    short = False
                    cash = (1+(short_start_price-reverse_price)/short_start_price)*trading_cash
                ...
                if all[t][3] < reverse_price:  # close price is lower than reverse_price
                    reverse_price = reverse_price - slide
                    #long over
                    long = False
                    cash = (1 - (long_start_price - reverse_price) / long_start_price) * trading_cash
...
if __name__ == '__main__':
    ...
    for i in range(length):
        value,counter = gmma.simulate(num=24 * 7 * 1 + 61, periods="1H", end_offset=3600 * 24 * 7 * (i+0))
        sum = sum + value
