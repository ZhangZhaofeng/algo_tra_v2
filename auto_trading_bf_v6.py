        ...
        if type == 'BUY' or type == 'buy':
            ...
            parameters =  [{ 'product_code' : product, 'condition_type' : 'STOP_LIMIT', 'side': 'BUY',
                           ...
                           {'product_code': product, 'condition_type': 'STOP', 'side': 'BUY', 'size': str(amount),
                            'trigger_price': str(insurance_price)},
                           ]
    ...
    def handel_partly_deal(self, order):
        ...
        if order['parent_order_state'] == 'COMPLETED' and order['executed_size'] != order['size']:
            ...
            if abs(order['executed_size'] - real_executed_size) > 0.001:
                predict.print_and_write('Maybe double executed, real: %f, executed: %f'%(real_executed_size, order['executed_size']))
                return (-1)
        else:
...
if __name__ == '__main__':
    ...
    if 1:
        ...
        order_places = {'exist': True, 'type': 'sell', 'id': 'JRF20180419-042645-529261', 'remain': 0.066,
                        'trade_price': 898850.0, 'slide': 200.0}
        tradeamount0 = 0
        position = 0.033 # 0
        holdflag = True
    elif 0:
