...
class GMMA:
    ...
    def simulate(self, num=100, periods="1m"):
        ...
        for t in range(62, len(all)):
            ...
            #buy_price = all[t][1]
            #sell_price = all[t][1]
            #sig_buy_p = result[1][t-1]
            #if sig_buy_p > buy_price:
            #    buy_price = sig_buy_p


            #sig_sell_p = result[3][t-1]
            #if sig_sell_p < sell_price:
            #    sell_price = sig_sell_p


            if all[t][2] > buy_price and hold == False:
                hold = True
                cash += shortbtc * (position - buy_price)
                longbtc = cash / (buy_price)
                shortbtc = 0.
                position = buy_price
                #cash = 0.
                buy_times += 1

            elif all[t][3] < sell_price and hold == True:
                hold = False
                cash += longbtc * (sell_price - position)
                position = sell_price
                longbtc = 0.
                shortbtc = cash / sell_price
                sell_times += 1
            ...
            value = all[t][4] * longbtc + all[t][4] * shortbtc
...
if __name__ == '__main__':
    ...
    gmma.simulate(num=24+61, periods="1H")